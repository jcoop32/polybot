#!/usr/bin/env python3
"""
PolySniper Paper Trader — $10 Bankroll Edition

Same logic as paper_trade.py but starts with a $10 balance that
grows or shrinks with each trade.  Trade size is the lesser of
$5 or the current balance.

Execution window: 15 seconds (configurable via --window).
Results logged to paper_trades_10.csv.

Usage:
    python paper_trade_10.py                  # Run until Ctrl+C
    python paper_trade_10.py --candles 20     # Run for 20 candles
    python paper_trade_10.py --window 30      # 30s execution window
    python paper_trade_10.py --no-reinvest    # Never bet with winnings
"""
...
MIN_BUY_PRICE = 0.10          # Don't buy if best ask is below this
SAFE_BUFFER_USD = 20.0        # BTC must be this far from candle open
DEFAULT_TRADE_WINDOW = 15     # Buy in last N seconds
MAX_TRADE_USD = 5.0           # Max trade size per candle (capped by balance)
MIN_ASK_SIZE_USD = 5.0        # Minimum orderbook depth at best ask
...
CSV_FILE = "paper_trades_10_v2.csv"
CSV_FILE_NOREINVEST = "paper_trades_10_noreinvest_v2.csv"
...
class Trade:
    ...
    balance_after: float        # Balance after this trade
    # ── Rich context fields ──
    ...
    def record(self, trade: Trade):
        ...
        if trade.decision.startswith("BUY"):
            ...
            if trade.actual_direction and trade.decision_delta != 0:
                # The bot picks the side matching the BTC delta at decision time
                ...
                async with session.ws_connect(
                    ...
                    while True:
                        try:
                            ...
                            # No message in PRICE_STALE_RECONNECT seconds — force reconnect
...
def print_waiting(market: dict, remaining: float, candle_open: float, stats: Stats,
                  trade_window: int = DEFAULT_TRADE_WINDOW, no_reinvest: bool = False):
    ...
    bal_c = GREEN if stats.balance >= STARTING_BALANCE else RED
    effective = min(stats.balance, STARTING_BALANCE) if no_reinvest else stats.balance
    bet_size = min(MAX_TRADE_USD, effective)

    clear()
    print(f"{B}{'═' * 64}{R}")
    label = "$10 PAPER TRADING (NO REINVEST)" if no_reinvest else "$10 PAPER TRADING"
    print(f"{B}  📝 POLYSNIPER — {label}{R}")
    print(f"{B}{'═' * 64}{R}")
    ...
    if stats.trades:
        ...
        if lt.decision.startswith("BUY"):
            ...
            if lt.btc_volatility > 0:
                print(f"  │ Volatility: ${lt.btc_volatility:,.2f} range")
            # Show what would have happened if we traded anyway
...
async def simulate_candle(
    ...
    Trade size = min(MAX_TRADE_USD, effective balance).
    If no_reinvest, effective balance is capped at STARTING_BALANCE.
    """
    event_start = market["event_start"]
    ...
    while True:
        ...
        if to_start <= 0:
            break
        print_waiting(market, 300, 0, stats, trade_window, no_reinvest)
    ...
    while True:
        ...
        # Show live status
        print_waiting(market, remaining, candle_open, stats, trade_window, no_reinvest)
            ...
            if stale_age > PRICE_STALE_RECONNECT:
                ...
                if abs_delta < SAFE_BUFFER_USD:
                    ...
                    if abs_delta >= SAFE_BUFFER_USD * 2:
                        confidence_score += 2    # Strong directional move
                    elif abs_delta >= SAFE_BUFFER_USD:
                        confidence_score += 1
                    if ask_price > 0 and ask_price <= 0.94:
                        confidence_score += 2    # Great price (6%+ implied edge)
                    elif ask_price <= 0.96:
                        confidence_score += 1    # Good price
                    if book_depth_usd >= 100:
                        confidence_score += 1    # Deep book
                    if opposing_ask_price > 0 and opposing_ask_price >= 0.15:
                        confidence_score += 1    # Other side isn't dead cheap
    ...
    if decision.startswith("BUY"):
        effective_bal = min(stats.balance, STARTING_BALANCE) if no_reinvest else stats.balance
...
def print_final_report(stats: Stats, csv_path: str = CSV_FILE):
    ...
    print()
    print(f"{B}{'═' * 64}{R}")
    print(f"{B}  📋 $10 PAPER TRADING — FINAL REPORT{R}")
    print(f"{B}{'═' * 64}{R}")
    print()
    ...
    # Shadow stats (show regardless of whether trades were placed)
    shadow_total = stats.shadow_wins + stats.shadow_losses
    if shadow_total > 0:
        shadow_wr = stats.shadow_wins / shadow_total * 100
        print()
    ...
    print()
    print(f"  {D}Full log saved to: {csv_path}{R}")
    print(f"{B}{'═' * 64}{R}")
    print()
...
async def main(max_candles: int | None = None, trade_window: int = DEFAULT_TRADE_WINDOW,
               no_reinvest: bool = False):
    """Run paper trader across multiple candles with $10 bankroll."""
    ...
    if no_reinvest:
        print(f"🔒 No-reinvest mode: bets capped at ${STARTING_BALANCE:.2f} effective balance")
    print()

    csv_path = CSV_FILE_NOREINVEST if no_reinvest else CSV_FILE
    init_csv(csv_path)
    stats = Stats()
    candle_num = 0
    ...
    try:
        async with aiohttp.ClientSession() as session:
            while True:
                ...
                trade = await simulate_candle(session, market, candle_num, stats, trade_window, no_reinvest)

                stats.record(trade)
                log_trade_csv(csv_path, trade, stats)
        ...
        price_task.cancel()
        print_final_report(stats, csv_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PolySniper Paper Trader — $10 Bankroll")
    parser.add_argument(
        "--candles", "-n",
        type=int,
        default=None,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Never bet with winnings — effective balance capped at starting amount",
    )
    args = parser.parse_args()

    try:
        asyncio.run(main(max_candles=args.candles, trade_window=args.window,
                         no_reinvest=args.no_reinvest))
